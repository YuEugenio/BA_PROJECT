"""
model_global_lora_free.py - 双路BioMedCLIP模型（无LoRA）
Two-stream BioMedCLIP model (no LoRA injection)
"""
...
class ClassificationHead(nn.Module):
    """
    分类头: 1024维 → 3类
    Classification Head: 1024-dim → 3 classes
    """

    def __init__(self, input_dim: int = 1024, num_classes: int = 3, dropout: float = 0.1):
        super().__init__()
...
class PESMultiTaskModel(nn.Module):
    ...
    def __init__(
        ...
        self.fused_dim = self.feature_dim * 2
        ...
        print("LoRA disabled: using plain BioMedCLIP backbone")
    ...
    def forward(
        ...
        fused = torch.cat([feat_implant, feat_control], dim=-1)
    ...
    def get_trainable_params(self) -> List[nn.Parameter]:
        """
        获取可训练参数（可选主干 + 分类头）
        Get trainable parameters (optional backbone + classification heads)
        """
        params = []
