...
class PESMultiTaskModel(nn.Module):
    ...
    def __init__(
        ...
        self.cross_attn_ic = nn.MultiheadAttention(
            embed_dim=self.feature_dim,
            num_heads=8,
            dropout=0.1,
            batch_first=True,
        )
        self.cross_attn_ci = nn.MultiheadAttention(
            embed_dim=self.feature_dim,
            num_heads=8,
            dropout=0.1,
            batch_first=True,
    ...
    def forward(
        ...
        token_implant = feat_implant.unsqueeze(1)
        token_control = feat_control.unsqueeze(1)
        token_global = feat_global.unsqueeze(1)

        attended_implant, _ = self.cross_attn_ic(
            query=token_implant,
            key=token_control,
            value=token_control,
        )
        attended_control, _ = self.cross_attn_ci(
            query=token_control,
            key=token_implant,
            value=token_implant,
        )
        ...
        fused = torch.cat([
            attended_implant.squeeze(1),
            attended_control.squeeze(1),
            token_global.squeeze(1)
        ], dim=-1)
    ...
    def get_trainable_params(self) -> List[nn.Parameter]:
        ...
        for name, param in self.classification_heads.named_parameters():
            params.append(param)
        for name, param in self.cross_attn_ic.named_parameters():
            params.append(param)
        for name, param in self.cross_attn_ci.named_parameters():
            params.append(param)
        return params
    ...
    def save_model(self, path: str):
        ...
        state_dict = {
            ...
            'cross_attn_ic': self.cross_attn_ic.state_dict(),
            'cross_attn_ci': self.cross_attn_ci.state_dict(),
        }
    ...
    def load_model(self, path: str):
        ...
        if 'cross_attn_ic' in state_dict:
            self.cross_attn_ic.load_state_dict(state_dict['cross_attn_ic'])
        if 'cross_attn_ci' in state_dict:
            self.cross_attn_ci.load_state_dict(state_dict['cross_attn_ci'])
