"""
train_legacy.py - Legacy training script with standardized outputs
"""
...
from dataset import create_dataloaders, summarize_dataset, build_data_items, patient_group_split, compute_label_distribution
from model_legacy_crossattention import create_model, PES_TASK_NAMES
...
def validate_epoch(
    ...
    results['avg_auc'] = np.mean([results[name]['auc'] for name in PES_TASK_NAMES])
...
def train_legacy(
    ...
    run_metadata = {
        'model_name': 'model_legacy_crossattention',
        'timestamp': timestamp,
        'data_dir': data_dir,
        'label_file': label_file,
        'epochs': epochs,
    ...
    with open(os.path.join(run_dir, 'data_summary.json'), 'w', encoding='utf-8') as f:
        json.dump({'dataset': data_summary, 'split': split_summary}, f, ensure_ascii=False, indent=2)
    ...
    try:
        from visualize_legacy_crossattention import plot_training_curves, plot_confusion_matrices, plot_auc_curves
...
def main():
    parser = argparse.ArgumentParser(description='Train PES Multi-Task Classification Model (Legacy CrossAttention)')
    ...
    parser.add_argument('--output_dir', type=str,
                        default='/data15/data15_5/yujun26/BA_PROJECT/legacy_crossattention/outputs',
