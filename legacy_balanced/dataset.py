...
def _task_class_counts(data_items: List[Dict]) -> Dict[str, np.ndarray]:
    """
    统计每个任务在给定样本集合中的类别计数。
    """
    counts = {task: np.zeros(3, dtype=np.int64) for task in PES_COLUMNS}
    for item in data_items:
        labels = item['labels']
        for index, task in enumerate(PES_COLUMNS):
...
def _split_quality_score(
    ...
    - 每类最小样本约束惩罚
    """
    train_counts = _task_class_counts(train_items)
    val_counts = _task_class_counts(val_items)
    ...
    for task in PES_COLUMNS:
        train_total = max(int(train_counts[task].sum()), 1)
        val_total = max(int(val_counts[task].sum()), 1)
        train_dist = train_counts[task] / train_total
        val_dist = val_counts[task] / val_total
        divergence += float(np.abs(train_dist - val_dist).sum())

        val_shortage = np.maximum(0, min_val_per_class - val_counts[task])
        train_shortage = np.maximum(0, min_train_per_class - train_counts[task])
        penalty += float(val_shortage.sum() + train_shortage.sum())

        min_count_report[task] = {
            'train_counts': train_counts[task].tolist(),
            'val_counts': val_counts[task].tolist(),
        }
    ...
    stats = {
        ...
        'penalty': penalty,
        'score': score,
        'counts': min_count_report,
    }
    return score, stats


def search_best_patient_group_split(
    data_items: List[Dict],
    test_size: float = 0.2,
    seed_base: int = 42,
    num_trials: int = 500,
    ...
    """
    在多个随机种子上搜索最优患者级划分，尽量平衡4个任务的train/val分布。
    """
    if len(data_items) == 0:
        return [], [], {
            'mode': 'auto_search',
    ...
    for offset in range(num_trials):
        ...
        train_items, val_items = patient_group_split(data_items, test_size=test_size, random_state=seed)
    ...
    return selected['train_items'], selected['val_items'], split_info
...
def create_dataloaders(
    ...
    rotation_deg: float = 8.0,
    blur_prob: float = 0.1
) -> Tuple[DataLoader, DataLoader, Dict[str, torch.Tensor], Dict]:
    """
    创建训练和验证数据加载器
    ...
    if auto_search_split:
        train_items, val_items, split_info = search_best_patient_group_split(
            data_items,
            test_size=test_size,
            seed_base=split_seed_base,
            num_trials=split_search_trials,
            min_val_per_class=min_val_per_class,
            min_train_per_class=min_train_per_class,
        )
        print(
            f"Auto split selected seed={split_info['seed']}, score={split_info['score']:.4f}, "
            f"feasible={split_info['feasible_trials']}/{split_info['trials']}"
        )
    else:
        ...
        split_info = {
            ...
            'score': score,
            'trials': 1,
            'feasible_trials': 1 if stats['penalty'] == 0 else 0,
            'split_stats': stats,
        }
    ...
    if use_weighted_sampler:
        ...
        for item in train_items:
            task_weights = [inv_freq[col][int(item['labels'][i])] for i, col in enumerate(PES_COLUMNS)]
            if sampler_strategy == 'avg_task':
                weight = float(np.mean(task_weights))
            elif sampler_strategy == 'sum_task':
                weight = float(np.sum(task_weights))
            else:
                weight = float(np.max(task_weights))
    ...
    split_info['sampler_strategy'] = sampler_strategy if use_weighted_sampler else 'disabled'
