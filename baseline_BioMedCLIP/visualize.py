"""
visualize.py - Visualization module for Global_lora_free
"""
...
from dataset import PES_COLUMNS
from model_baseline_biomedclip import PES_TASK_NAMES
...
def plot_class_distribution(
    ...
    for index, column_name in enumerate(PES_COLUMNS):
        ax = axes[index]
        ...
        ax.set_xlabel('Class')
        ax.set_ylabel('Count')
        ax.set_title(display_task_name(column_name))
        ax.set_xticks([0, 1, 2])
    ...
    plt.suptitle('Class Distribution for 4 PES Tasks', fontsize=15, y=1.02)
    plt.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(description='Visualization for Global_lora_free')
    parser.add_argument('--mode', type=str, required=True,
                        choices=['training', 'confusion', 'auc', 'distribution'],
    ...
    if args.mode == 'training':
        ...
        plot_class_distribution(args.label_file, args.output, show)
