"""
Baseline model: ImageNet-pretrained ResNet50 + cross-attention fusion + linear heads.
"""
...
class PESBaselineModel(nn.Module):
    """
    Baseline-style architecture:
    - two-stream feature extraction with ImageNet-pretrained ResNet50
    - bidirectional cross-attention fusion
    - single linear classifier per task
    """

    def __init__(self, freeze_backbone: bool = False, attn_heads: int = 8, device: str = 'cuda'):
        super().__init__()
        self.device = device
...
def create_model(device: str = 'cuda') -> Tuple[PESBaselineModel, callable]:
    model = PESBaselineModel(device=device)
